...
#
# Walks .claude/agent-memory/*/MEMORY.md (relative to the current project
# root, i.e. the cwd when the session starts) and prints the concatenated
# content to stdout, with one section per agent. Claude Code injects this
...
#
# Disable per-project: set CLAUDE_RESEARCH_DISABLE_MEMORY_LOAD=1.

set -u

if [[ "${CLAUDE_RESEARCH_DISABLE_MEMORY_LOAD:-0}" == "1" ]]; then
  exit 0
fi

memory_root=".claude/agent-memory"
...
echo "# Agent Memory (auto-loaded by oh-my-claudecode-research)"
echo ""
...
while IFS= read -r f; do
  [[ -z "$f" ]] && continue
  # Extract agent name from path: .claude/agent-memory/<agent>/MEMORY.md
