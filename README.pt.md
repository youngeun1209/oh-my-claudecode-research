...
**Orquestração multi-agente para o Codex — edição de pesquisa. Curva de aprendizado zero.**

_Não aprenda ferramentas de pesquisa. Apenas use o OMXR._

OMXR é um workspace de pesquisa para o Codex: seis agentes — `@supervisor`, `@analysis-implementer`, `@paper-writer`, `@figure-descriptor`, `@reviewer`, `@literature-curator` — com quem você trabalha lado a lado em hipótese, análise, escrita, figuras, citações, revisão. Seis engines de orquestração automatizam os loops comuns quando você quer hands-off. Combine com [oh-my-codex](https://github.com/Yeachan-Heo/oh-my-codex) quando precisar de orquestração genérica em cima (retries, paralelismo, controle de orçamento).
...
**Se você está instalando OMXR pela primeira vez** — fluxo marketplace (recomendado). São comandos slash do Codex, digite **um de cada vez**:
...
**Se o OMXR já está instalado e você quer atualizar** — rode estes dois comandos slash, um de cada vez:
...
O primeiro só atualiza os metadados do marketplace; o segundo é o que realmente puxa os arquivos novos do plugin. OMXR segue o `main`, então cada commit novo é tratado como uma versão nova. O estado do seu projeto (AGENTS.md, memória dos agentes, settings) não é tocado — não precisa rodar Step 2 de novo.

**Step 2: Configuração**

**Só precisa rodar uma vez por projeto.** Dentro de uma sessão do Codex no seu projeto de pesquisa, rode os comandos slash **um de cada vez**:
...
`$omxr-setup` planta a infraestrutura — blocos vazios `## Project context` / `## Research stack` / `## Language preference` no `AGENTS.md`, `.omx/omxr/agent-memory/<agent>/MEMORY.md` para todos os 6 agentes, `paper/references.bib` + `./references.csv` vazios para o literature-curator, e uma hook/check readiness de permissões curada. **Nenhuma pergunta sobre sua pesquisa.**
...
Se você rodar `$start-research` antes de `$omxr-setup`, ele oferece rodar `$omxr-setup` primeiro. Campos científicos pulados são salvos como `[TBD: <nota curta>]` — nunca inventados — para que `@supervisor` saiba que precisa fazer follow-up. Se pular ambos, o hook `setup-nudge` no SessionStart imprime um lembrete de uma linha a cada sessão até você inicializar (suprima com `CODEX_RESEARCH_DISABLE_SETUP_NUDGE=1`).
...
### 4 workflow commands (parametrizados via o AGENTS.md do seu projeto)

| Command | What it does |
|---|---|
| `$omxr-setup` | Estilo instalação: cria blocos marcadores vazios no `AGENTS.md`, diretórios `.omx/omxr/agent-memory/`, `references.bib`/`references.csv` vazios, e uma hook/check readiness de permissões curada. **Não faz perguntas sobre sua pesquisa.** Rode uma vez por projeto. |
| `$start-research [minimal\|neuro-fmri]` | Estilo entrevista: preenche os placeholders no `AGENTS.md` (working title, hipótese, venue alvo, datasets, fio narrativo), opcionalmente aplica um preset à memória dos agentes, faz scaffold do diretório LaTeX do manuscrito (via skill `manuscript-scaffold`, com template do journal + clone Overleaf opcional). Oferece rodar `$omxr-setup` antes se ainda não tiver sido feito. |
...
Os 4 comandos setup/workflow são thin dispatchers — cada um encaminha `$ARGUMENTS` para uma skill correspondente. `cropfig`, `verify-citation`, `manuscript-scaffold` também podem ser invocados de forma independente. **Além disso** 1 primitive (`orchestrate` — interno, compõe via 4 fases) + 6 engine skills que suportam os 6 comandos de orquestração; walkthrough completo em [`wiki/Using-Orchestration.md`](wiki/Using-Orchestration.md). A tabela abaixo cobre as 7 skills setup/workflow.

| Skill | What it does |
|---|---|
| `omxr-setup` | Suporta `$omxr-setup`. Estilo instalação: scaffold de blocos marcadores no `AGENTS.md`, diretórios agent-memory, arquivos de bibliografia, hook/check readiness de permissões curada. |
| `start-research` | Suporta `$start-research`. Init estilo entrevista do primeiro projeto: preenche os placeholders já scaffoldados no `AGENTS.md`, opcionalmente aplica uma preset overlay, delega o manuscript scaffold para `manuscript-scaffold`. |
...
| `memory-load` | `SessionStart` | Auto-injeta `.omx/omxr/agent-memory/*/MEMORY.md` no contexto da sessão. |
...
| `setup-nudge` | `SessionStart` | Cutucada de uma linha não bloqueante para rodar `$omxr-setup` e depois `$start-research` se o AGENTS.md não tiver os blocos `## Project context` ou `## Research stack`. |
...
- **[Standalone Usage](wiki/Standalone-Usage.md)** — usando o OMXR sozinho, walkthrough completo
- **[With OMX](wiki/With-OMX.md)** — full stack: instalação OMXR + OMX companion
- **[Agents](wiki/Agents.md)** | **[Commands](wiki/Commands.md)** | **[Hooks](wiki/Hooks.md)** — referências
- **[OMX Tool Reference](wiki/OMX-Tool-Reference.md)** — 47 ferramentas OMX MCP mapeadas para estágios de pesquisa
...
OMXR trata [`oh-my-codex`](https://github.com/Yeachan-Heo/oh-my-codex) como um *companion*, não uma dependência. Com o OMX instalado ao lado, os componentes a seguir se encaixam naturalmente em fluxos de pesquisa. Escolha os relevantes para seu projeto — você não precisa usar todos.
...
| `@document-specialist` agent | Pesquisa de literatura mais pesada apoiada pelo Context Hub do OMX (fetches cacheados, notas estruturadas). Use junto a `@literature-curator` quando precisar de um deep dive em escala de survey; o curator do OMXR cuida sozinho da resolução de citação por claim e da gestão BibTeX/summary-table. |
| `@verifier` agent | Verificação de completude baseada em evidência — rejeita afirmações "should work" sem output de teste fresco. |
| `@tracer` agent + `/oh-my-codex:trace` | Ranking de hipóteses competidoras baseado em evidência + disconfirmation. Mapeia para validação de methods/results. |
...
| `/oh-my-codex:autoresearch` skill | Loop de iteração bounded evaluator-driven com JSON + decision log por iteração. |
| `/oh-my-codex:deep-interview` skill | Clarificação socrática de objetivos de pesquisa vagos em hipóteses testáveis. |
| Skills de orquestração OMX (`ralph`, `team`, `autopilot`, `ralplan`, `ultraqa`, `autoresearch`, …) | Orquestradores multi-iteração / paralelo / consenso para runs de análise, varreduras de literatura, revisões must-finish. Veja [`wiki/With-OMX.md#recipes--pairing-omxr-with-omc`](wiki/With-OMX.md#recipes--pairing-omxr-with-omc) para 5 receitas práticas. |
...
Instale o OMX ao lado via Codex marketplace, ou `npm i -g oh-my-codex`. Mapeamento completo: [`wiki/With-OMX.md`](wiki/With-OMX.md) + [`wiki/OMX-Tool-Reference.md`](wiki/OMX-Tool-Reference.md)
...
- Diretiva de idioma **English-first** em todos os agentes (padrão de override no AGENTS.md)
