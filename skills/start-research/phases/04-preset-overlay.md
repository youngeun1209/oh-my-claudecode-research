...
2. If yes, compare `.omx/omxr/agent-memory/<agent>/MEMORY.md` (the user's project) against the canonical template at `templates/MEMORY.template.md`:
...
For `neuro-fmri`, the plugin ships an agent body overlay at `examples/neuro-fmri/agents/analysis-implementer.md`. When the plugin is installed via `/plugin install`, Codex loads the **plugin core** `agents/analysis-implementer.md`, not the preset overlay. The overlay is included in `examples/` as a reference for users who want to copy it manually into `.codex/agents/analysis-implementer.md` in their project, which overrides the plugin core.

Mention this in the phase 7 report:

> "Neuro-fMRI body overlay available at `examples/neuro-fmri/agents/analysis-implementer.md` if you want neuro-flavored agent behavior. Copy to `.codex/agents/analysis-implementer.md` to apply."
