---
name: start-research
description: Interview-driven first-research-project initialization — asks about working title, field, hypothesis, target venue, datasets, narrative spine; fills the CLAUDE.md placeholders that /omcr-setup scaffolded; applies an optional domain preset to agent memory (when the existing MEMORY.md is still canonical-template); scaffolds the LaTeX manuscript directory via the manuscript-scaffold skill. Requires /omcr-setup to have run first — will offer to run it if not. Safe to re-run; never overwrites filled-in answers, modified MEMORY.md, or existing manuscript content.
---
...
Record as `preset_arg` so phase 2 / phase 4 can consume it.

## Phase execution

Execute phases sequentially. For each phase, read the linked file and follow its instructions exactly. If an earlier phase reports an early stop (e.g. user cancelled, `/omcr-setup` not run and user declined to run it), do **not** skip phase 6 (the report) — surface the stop reason there.
...
3. **Phase 3 — Fill CLAUDE.md.** Read [`phases/03-fill-claude-md.md`](phases/03-fill-claude-md.md).
4. **Phase 4 — Preset overlay (agent memory).** Read [`phases/04-preset-overlay.md`](phases/04-preset-overlay.md).
5. **Phase 5 — Manuscript scaffold (delegated).** Read [`phases/05-manuscript-scaffold.md`](phases/05-manuscript-scaffold.md). This phase delegates to the `manuscript-scaffold` skill.
...
- Phase 2 surfaces existing `CLAUDE.md` values as defaults. The user can confirm-all to apply only newly-added fields.
...
- Phase 5 delegates to `manuscript-scaffold`, which has its own existing-content guard.
...
After phase 6's report, end by recommending the user run `@supervisor where are we?` for the first real conversation.
