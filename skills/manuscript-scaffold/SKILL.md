...
<Do_Not_Use_When>
- The user wants only journal-template metadata captured in `CLAUDE.md` without creating files (no skeleton, no commit) — that is a `CLAUDE.md` edit, not this skill.
...
<Configuration>
Inputs are resolved by the caller (typically `/start-research`) from the project's `CLAUDE.md` blocks, or passed directly when the skill is invoked standalone.
...
- **Overleaf token never written to a tracked file.** Phase 3 stores the token only in `git credential-store` (scoped to `git.overleaf.com`) or `~/.netrc`. Never write the token into `CLAUDE.md`, agent memory, the project repo, or this plugin. When referencing the token in any output, mask all but the last 4 chars.
