...
How to actually USE OMXR's 6 orchestration engines after `$omxr-setup` and `$start-research` are done. This page is the "what do I type" reference; for internal mechanics see [Orchestration-Model](Orchestration-Model.md), for OMX composition see [Orchestration-Comparison](Orchestration-Comparison.md), and for the autonomous mode deep-dive see [Autonomous-Drive](Autonomous-Drive.md).

## 3 levels of automation

OMXR layers from manual to fully autonomous. Pick the level that matches what you want right now:
...
One engine drives one workflow to completion. You start it; the engine reads/writes state under `.omx/state/omxr/`, dispatches subagents, loops, and reports back with a DONE / CONTINUE / BLOCKED / HALT verdict.
...
**When to use:** long working session, end-of-day "drive everything forward", or when you want OMXR to find the bottleneck for you.
...
```
Expose .codex-plugin/plugin.json through Codex plugin discovery
npm install -g oh-my-codex-research
```

In your project root:

```
$omxr-setup        # scaffold AGENTS.md blocks + agent-memory + .omx/state/omxr/
$start-research   # interview: hypothesis, venue, datasets, outline path
```

After this, `.omx/state/omxr/` exists with 5 empty state JSONs: `paper.json`, `reviews.json`, `citations.json`, `figures.json`, `rebuttals.json` (plus `_run-log.jsonl`).
...
All orchestration state lives under `.omx/state/omxr/` in your project (gitignored):
...
Canonical schemas: [`develop/example-state/`](../develop/example-state/) (tracked as reference; copied into your project by `$omxr-setup`). Engines never overwrite human-curated fields; they only add/append.
...
## When to compose with OMX

OMXR's engines compose with upstream OMX's generic orchestrators when you want extra power:

| Need | Composition |
|---|---|
| Retry an engine with a verifier | `/oh-my-codex:ralph` wrapping OMXR engine |
| True parallel literature scan at scale | `omx team N:literature-curator` instead of `$literature-sweep --parallel` |
| Try N different writing styles | `/oh-my-codex:ultraqa` over `$iterate-revision` |
| Budget-tracked autonomous drive with decision log | `/oh-my-codex:autopilot` wrapping `$supervisor-drive --auto` |
| Cross-check a critical claim | `@oh-my-codex:verifier` after `$iterate-revision` DONE |

Full matrix + 5 worked recipes: [Orchestration-Comparison](Orchestration-Comparison.md) and [With-OMX](With-OMX.md).
...
- [Orchestration-Comparison](Orchestration-Comparison.md) — OMXR alone vs OMXR + OMX, decision tree, cost table
