...
6 名のリサーチチームエージェント + 6 つのオーケストレーションエンジン + 7 つの setup/workflow/utility コマンド + 18 のスキル + 4 つの軽量フック。

> **ステータス: v0.1。** Breaking change が入る可能性があります。フィードバックや PR を歓迎します。

> **完全なドキュメント:** [`wiki/Home.md`](wiki/Home.md)

> **Claude Code ではなく Codex CLI を使っていますか?** Codex 版は [oh-my-codex-research](https://github.com/youngeun1209/oh-my-codex-research) (OMXR) にあります ── 同じ 6 エージェントのリサーチチームを OpenAI Codex CLI 向けに移植したものです。
...
### 7 slash commands (プロジェクトの CLAUDE.md でパラメータ化)
...
| `/session-start [light\|full]` | 読み取り専用のオリエンテーション: プロジェクトのコーパス(CLAUDE.md、outline、MEMORY、wiki ランディングページ)を読み、サマリー + 正直なステータスを報告。副作用ゼロ。light/full モードあり。 |
| `/save-session-log [slug]` | セッション(依頼内容、行った作業、触ったファイル、決定事項、次のステップ)を忠実に記録した日付付きログを session-logs ディレクトリに書き込み、その後、確定した知見を wiki に手術的に蒸留。 |
| `/update-version [@new files]` | outline や figure deck がバンプされたとき(v4→v5)、新しいファイル名をすべての現行参照箇所に反映。廃止されたアーカイブの削除も提案(確認ゲート付き)。旧値は `## Research stack` から読み取る。 |

### 18 skills

7 つの setup/workflow/utility スラッシュコマンドは thin dispatcher ── 各自 `$ARGUMENTS` を同名スキルに転送します。`cropfig`、`verify-citation`、`manuscript-scaffold`、`paper-ingest` は単独でも呼び出し可能。**さらに** 1 つの primitive(`orchestrate` ── 内部用、4 つの phase で構成) + 6 つのオーケストレーションコマンドを支える engine skill。フルウォークスルーは [`wiki/Using-Orchestration.md`](wiki/Using-Orchestration.md)。下表は 11 の setup/workflow/utility スキルをカバーします。
...
| `paper-ingest` | 読んだ論文(PDF / DOI / URL)を二分割の **reading library** に取り込む ── プロジェクトに依存しないサマリー + crop 済みのメイン figure + `index.csv` 行を生成し、続いて relevance でゲートされたプロジェクト用メモを作成。`verify-citation` + `cropfig` を再利用。manuscript の BibTeX とは別管理。単独実行。詳細は [Reading-Library](wiki/Reading-Library.md)。 |
| `session-start` | `/session-start` を支える。読み取り専用のプロジェクトオリエンテーション(コーパス読み込み + ステータス報告。light/full モードあり)。 |
| `save-session-log` | `/save-session-log` を支える。日付付きセッション記録 + 確定した知見の wiki への手術的蒸留。 |
| `update-version` | `/update-version` を支える。outline/deck のバージョンバンプをすべての現行参照箇所に反映。過去の日付付き記録は凍結したまま。 |
...
- **[Reading Library](wiki/Reading-Library.md)** — `paper-ingest`: 読んだ論文をファイルする(manuscript の BibTeX とは別管理)
