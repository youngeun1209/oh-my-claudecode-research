...
**Điều phối đa tác nhân cho Codex — phiên bản dành cho nghiên cứu. Không cần học.**

_Đừng học công cụ nghiên cứu. Cứ dùng OMXR._

OMXR là một không gian làm việc nghiên cứu cho Codex: sáu agent — `@supervisor`, `@analysis-implementer`, `@paper-writer`, `@figure-descriptor`, `@reviewer`, `@literature-curator` — bạn cùng làm việc trên giả thuyết, phân tích, viết, figure, citation, review. Sáu engine điều phối tự động hóa các vòng lặp phổ biến khi bạn muốn hands-off. Kết hợp với [oh-my-codex](https://github.com/Yeachan-Heo/oh-my-codex) khi cần điều phối tổng quát ở tầng trên (retry, parallel, budget tracking).
...
**Nếu bạn cài OMXR lần đầu** — luồng marketplace (khuyến nghị). Đây là lệnh slash của Codex, gõ **từng cái một**:
...
**Nếu OMXR đã cài và bạn muốn update** — chạy hai lệnh slash sau, từng cái một:
...
Lệnh đầu chỉ refresh metadata của marketplace; lệnh thứ hai mới thực sự kéo file plugin mới về. OMXR theo nhánh `main`, nên mỗi commit mới được coi như một phiên bản mới. Trạng thái dự án của bạn (AGENTS.md, agent memory, settings) không bị động vào — không cần chạy lại Step 2.

**Step 2: Khởi tạo**

**Chỉ cần làm một lần cho mỗi dự án.** Trong một phiên Codex ở dự án nghiên cứu của bạn, chạy lệnh slash **từng cái một**:
...
`$omxr-setup` lát hạ tầng — các block trống `## Project context` / `## Research stack` / `## Language preference` trong `AGENTS.md`, `.omx/omxr/agent-memory/<agent>/MEMORY.md` cho cả 6 agent, `paper/references.bib` + `./references.csv` trống cho literature-curator, và một hook/check readiness quyền được tuyển chọn.**Không hỏi về nghiên cứu của bạn.**
...
Nếu bạn chạy `$start-research` trước `$omxr-setup`, nó đề nghị chạy `$omxr-setup` trước. Các field khoa học bị bỏ qua được lưu thành `[TBD: <ghi chú ngắn>]` — không bao giờ bịa — để `@supervisor` biết cần follow up. Nếu bỏ qua cả hai, hook `setup-nudge` ở SessionStart in một dòng nhắc mỗi phiên cho tới khi bạn init (tắt bằng `CODEX_RESEARCH_DISABLE_SETUP_NUDGE=1`).
...
### 4 workflow commands (tham số hóa qua AGENTS.md của dự án)

| Command | What it does |
|---|---|
| `$omxr-setup` | Kiểu cài đặt: scaffold các block marker rỗng trong `AGENTS.md`, thư mục `.omx/omxr/agent-memory/`, `references.bib`/`references.csv` rỗng, và một hook/check readiness quyền được tuyển chọn.**Không hỏi gì về nghiên cứu của bạn.** Chạy một lần mỗi dự án. |
| `$start-research [minimal\|neuro-fmri]` | Kiểu phỏng vấn: điền các placeholder trong `AGENTS.md` (working title, giả thuyết, venue mục tiêu, dataset, mạch truyện), tùy chọn áp preset vào agent memory, scaffold thư mục LaTeX manuscript (qua skill `manuscript-scaffold`, kèm template tạp chí + clone Overleaf tùy chọn). Đề nghị chạy `$omxr-setup` trước nếu chưa chạy. |
...
| Skill | What it does |
|---|---|
| `omxr-setup` | Backing `$omxr-setup`. Kiểu cài đặt: scaffold block marker trong `AGENTS.md`, thư mục agent-memory, file bibliography, hook/check readiness quyền được tuyển chọn. |
| `start-research` | Backing `$start-research`. Init dự án đầu kiểu phỏng vấn: điền các placeholder đã scaffold trong `AGENTS.md`, tùy chọn áp preset overlay, ủy quyền manuscript scaffold cho `manuscript-scaffold`. |
...
| `memory-load` | `SessionStart` | Tự inject `.omx/omxr/agent-memory/*/MEMORY.md` vào context phiên. |
| `citation-warn` | `PostToolUse:Write\|Edit` | Cảnh báo heuristic không chặn khi markdown manuscript có đoạn không trích dẫn. |
| `setup-nudge` | `SessionStart` | Nudge một dòng không chặn để chạy `$omxr-setup` rồi `$start-research` nếu AGENTS.md thiếu block `## Project context` hoặc `## Research stack`. |
...
- **[Standalone Usage](wiki/Standalone-Usage.md)** — dùng OMXR một mình, hướng dẫn đầy đủ
- **[With OMX](wiki/With-OMX.md)** — full stack: cài OMXR + OMX companion
- **[Agents](wiki/Agents.md)** | **[Commands](wiki/Commands.md)** | **[Hooks](wiki/Hooks.md)** — tham chiếu
- **[OMX Tool Reference](wiki/OMX-Tool-Reference.md)** — 47 công cụ OMX MCP được map vào các giai đoạn nghiên cứu
...
OMXR coi [`oh-my-codex`](https://github.com/Yeachan-Heo/oh-my-codex) là *companion*, không phải dependency. Khi cài OMX song song, các component sau ăn khớp tự nhiên vào quy trình nghiên cứu. Chọn cái phù hợp dự án bạn — không cần dùng hết.
...
| `@document-specialist` agent | Nghiên cứu literature nặng đô được hậu thuẫn bởi Context Hub của OMX (fetch cache, ghi chú có cấu trúc). Dùng cùng `@literature-curator` khi cần deep dive cỡ survey; curator của OMXR tự xử lý resolution citation per-claim và quản lý BibTeX/summary-table. |
| `@verifier` agent | Kiểm tra hoàn thành dựa trên bằng chứng — từ chối tuyên bố "should work" không có output test mới. |
| `@tracer` agent + `/oh-my-codex:trace` | Xếp hạng giả thuyết cạnh tranh dựa trên bằng chứng + disconfirmation. Map vào xác thực methods/results. |
...
| `/oh-my-codex:autoresearch` skill | Vòng lặp lặp bounded evaluator-driven với JSON + decision log mỗi iteration. |
| `/oh-my-codex:deep-interview` skill | Làm rõ kiểu Socratic mục tiêu nghiên cứu mơ hồ thành giả thuyết kiểm tra được. |
| Skill điều phối OMX (`ralph`, `team`, `autopilot`, `ralplan`, `ultraqa`, `autoresearch`, …) | Bộ điều phối multi-iteration / parallel / consensus cho lần chạy phân tích, quét literature, revision phải hoàn thành. Xem [`wiki/With-OMX.md#recipes--pairing-omxr-with-omc`](wiki/With-OMX.md#recipes--pairing-omxr-with-omc) cho 5 công thức thực tế. |
...
Cài OMX song song qua Codex marketplace, hoặc `npm i -g oh-my-codex`. Mapping đầy đủ: [`wiki/With-OMX.md`](wiki/With-OMX.md) + [`wiki/OMX-Tool-Reference.md`](wiki/OMX-Tool-Reference.md)
...
- Directive ngôn ngữ **English-first** trên mọi agent (pattern override-in-AGENTS.md)
