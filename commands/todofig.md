...
|---|---|---|
| `Figure PNG dir` | `<dirname(Deck file)>/png/` | Where the cropped per-slide PNGs live. `/todofig` reads these to inspect figure state. Populated by the `cropfig` skill. |
...
Figure refresh is owned by the `cropfig` skill, not `/todofig`. Run `cropfig` separately if the PNG dir is stale or empty.
