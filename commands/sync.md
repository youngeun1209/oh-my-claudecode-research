...
Thin dispatcher — the full status-reconciliation workflow lives in the `sync` skill so the slash-command surface stays small and the workflow can also be invoked outside `/sync`.
